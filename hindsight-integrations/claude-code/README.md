...
| `retainMaxChars` | `HINDSIGHT_RETAIN_MAX_CHARS` | `30000` | Maximum characters sent per retain payload. Oversized transcripts keep the session head and latest tail with a truncation marker. Set `0` to disable the cap. |
| `retainRoles` | — | `["user", "assistant"]` | Which message roles to include in the retained transcript. |
| `retainToolCalls` | — | `false` | Whether to include tool calls (function invocations and results) in the retained transcript. Captures structured actions like file reads, searches, and code edits. |
